...
Is it feasable to build a tool that pulls in live audio data from a police scanner and outputs locations of emergencies on a map?
...
**ancillary_csv** - Scapred street names and police codes
**enhanced_audio** - Enhanced audio files
**geocodes** - Latitude and Logitude data for Boston and Watertown
...
36 hours of police scanner audio files were aquired from Broadcstify via the 'BroadcastifyArchive API'. All street names for the greater Boston area were scraped from 'geographic.org' using 'bs4' 'BeautifulSoup' library. 

We found that the raw audio files were rough had a lot of static noise required cleaning. The raw audio files were cleaned by running them through 'Dolby's' audio cleaning API.  The cleaned audio files were then split on silence leaving only portions with an audible voice using the 'pydub' 'split_on_silence' audio processing library. We then ran the cleaned and split audio files through 'Google Cloud' 'speech_v1p1beta1' speech to text function which outputs trasnscribed text. 

The trascribed text was then processed using 'NLTK' 'RegexpTokenizer' to break the text up into individual words. These words were then cross referenced with the list of greater Boston area street names using 'spacy' 'Matcher' function to determine if any of the words from the transcript matched street names. The transcript was then processed again to pull out possible street numbers using the 'usaddress' library. The list of possible street names ('Hanover') and the list of possible street numbers ('213') were concatenated to creeat a list of possible full addresses (213 Hanover, etc.).
